-- Zero-downtime VALIDATE migration
-- Confirms there is no drift between legacy camelCase and snake_case columns.
DO $$
DECLARE
  r RECORD;
  drift_count bigint;
BEGIN
  ...
  LOOP
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=r.schema_name AND table_name=r.table_name AND column_name=r.old_col)
       AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=r.schema_name AND table_name=r.table_name AND column_name=r.new_col) THEN
      EXECUTE format('SELECT count(*) FROM %I.%I WHERE %I IS DISTINCT FROM %I', r.schema_name, r.table_name, r.old_col, r.new_col) INTO drift_count;
      IF drift_count > 0 THEN
        RAISE EXCEPTION 'Drift detected: %.% (% -> %) has % mismatched rows', r.schema_name, r.table_name, r.old_col, r.new_col, drift_count;
      END IF;
    END IF;
  END LOOP;
END $$;

-- Total mapping pairs checked: 211
