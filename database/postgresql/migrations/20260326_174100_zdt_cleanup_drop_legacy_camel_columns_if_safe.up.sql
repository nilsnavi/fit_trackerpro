-- Hard cleanup (optional): drop legacy camelCase columns where safe
-- Safety rules:
-- 1) Drop only if both legacy camelCase and snake_case columns exist.
-- 2) Drop only if legacy column has no dependencies in pg_depend.
...
BEGIN
  FOR r IN
    ...
    ) AS t(schema_name, table_name, legacy_col, snake_col)
  LOOP
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=r.schema_name AND table_name=r.table_name AND column_name=r.legacy_col) THEN
      RAISE NOTICE 'Skip %.%: legacy column % already removed', r.schema_name, r.table_name, r.legacy_col;
      CONTINUE;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=r.schema_name AND table_name=r.table_name AND column_name=r.snake_col) THEN
      RAISE NOTICE 'Skip %.%: snake column % missing', r.schema_name, r.table_name, r.snake_col;
      CONTINUE;
    END IF;

    relid := to_regclass(format('%I.%I', r.schema_name, r.table_name));
    IF relid IS NULL THEN
      RAISE NOTICE 'Skip %.%: table not found', r.schema_name, r.table_name;
      CONTINUE;
    END IF;
    ...
    WHERE a.attrelid = relid
      AND a.attname = r.legacy_col
      AND a.attnum > 0
      AND NOT a.attisdropped;

    IF legacy_attnum IS NULL THEN
      RAISE NOTICE 'Skip %.%: legacy column % not found in pg_attribute', r.schema_name, r.table_name, r.legacy_col;
      CONTINUE;
    END IF;
    ...
    INTO dep_count
    FROM pg_depend d
    WHERE d.refobjid = relid
      AND d.refobjsubid = legacy_attnum
      AND d.deptype IN ('n','a','i','e','x');

    IF dep_count > 0 THEN
      RAISE NOTICE 'Skip %.%: legacy column % has % dependencies', r.schema_name, r.table_name, r.legacy_col, dep_count;
      CONTINUE;
    END IF;

    EXECUTE format('ALTER TABLE %I.%I DROP COLUMN %I', r.schema_name, r.table_name, r.legacy_col);
    RAISE NOTICE 'Dropped %.% legacy column %', r.schema_name, r.table_name, r.legacy_col;
  END LOOP;
END $$;

COMMIT;

-- Total mapping pairs considered: 211
