...
export function MuscleLoadTable({
    dateFrom,
    dateTo,
    pageSize = 10,
    muscleGroup,
        ...
        return (
            <div className="rounded-xl border border-dashed border-border bg-telegram-secondary-bg/60 px-4 py-8 text-center">
                ...
                <p className="mt-2 text-sm text-telegram-hint">
                    Нет данных о мышечной нагрузке за выбранный период
                </p>
            </div>
        )
    }
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="hidden rounded-t-2xl bg-telegram-secondary-bg/80 px-4 py-2 sm:block">
                <div className="grid grid-cols-3 gap-2 text-xs font-medium text-telegram-hint">
                    <span>Дата</span>
                    <span>Мышечная группа</span>
            ...
            <div className="divide-y divide-border/50 overflow-hidden rounded-2xl bg-telegram-secondary-bg">
                {items.map((item, index) => (
                    <MuscleLoadTableRow
                        key={`${item.date}-${item.muscleGroup}-${index}`}
                        item={item}
...
function MuscleLoadTableRow({
    item,
    isLast,
}: {
    item: ApiMuscleLoadTableEntry
    isLast: boolean
}) {
    ...
    return (
        ...
        >
            ...
            <div className="sm:hidden">
                ...
                <p className="mt-1.5 text-xs text-telegram-hint">
                    {item.muscleGroup}
                </p>
            </div>

            {/* Desktop Layout */}
            <div className="hidden sm:grid sm:grid-cols-3 sm:gap-2 sm:text-sm">
                <span className="text-telegram-text" title={fullDate}>
                    {formattedDate}
                </span>
                <span className="text-telegram-text">{item.muscleGroup}</span>
