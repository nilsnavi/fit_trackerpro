import { Dumbbell, Heart, Timer, FileText, ChevronUp, ChevronDown, Edit2, Trash2 } from 'lucide-react'
...
export function TemplateBlockCard({
    ...
    return (
        <div className={cn('tg-card flex items-center gap-3 p-3')}>
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-telegram-hint truncate">{summary || block.config?.note || 'Без настроек'}</p>
                {reorderHint ? (
                    <p className="mt-0.5 text-[10px] text-telegram-hint/90">{reorderHint}</p>
                ) : null}
            </div>
