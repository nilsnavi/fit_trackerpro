...
type TimerSizeConfig = (typeof sizeStyles)[keyof typeof sizeStyles];
...
const TimerCircular: React.FC<{
    className?: string;
    sizeConfig: TimerSizeConfig;
    radius: number;
    circumference: number;
    strokeDashoffset: number;
    timerState: TimerState;
    timeLeft: number;
}> = ({ className, sizeConfig, radius, circumference, strokeDashoffset, timerState, timeLeft }) => (
    <div
        className={cn('relative inline-flex items-center justify-center', className)}
        style={{ width: sizeConfig.width, height: sizeConfig.width }}
    >
        {/* Фоновый круг */}
        <svg className="transform -rotate-90" width={sizeConfig.width} height={sizeConfig.width}>
            <circle
...
const TimerDigital: React.FC<{
    className?: string;
    timerState: TimerState;
    timeLeft: number;
    progress: number;
}> = ({ className, timerState, timeLeft, progress }) => (
...
export const Timer: React.FC<TimerProps> = ({
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            {showControls && (
                <TimerControls timerState={timerState} reset={reset} start={start} pause={pause} />
            )}
        </div>
    );
};
