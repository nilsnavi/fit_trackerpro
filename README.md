...
- `/api/v1/workouts/*`
- `/api/v1/progression/*` — политики, рекомендации, lifecycle и история прогрессии. Принятая (`accepted`/`modified`) рекомендация подставляется в новую сессию, запущенную из того же шаблона, а также в «Повторить тренировку», если повторяемая сессия была запущена из шаблона: черновик открывается на принятом весе (например, `82.5` вместо `80`), при этом сам шаблон не изменяется, а непринятая (`generated`) рекомендация ничего не подставляет.
- `/api/v1/exercises/*`
- `/api/v1/users/*`
- `/api/v1/users/auth/*`
