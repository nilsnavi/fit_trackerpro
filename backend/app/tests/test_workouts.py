...
class TestWeightRecommendation:
    async def test_weight_recommendation_requires_auth(self, client: AsyncClient):
        """Test that weight recommendation endpoint requires authentication"""
        r = await client.get("/api/v1/workouts/sessions/1/exercises/1/weight-recommendation")
        assert r.status_code == 401

    async def test_weight_recommendation_no_data_returns_message(
        self,
        authenticated_client: AsyncClient,
    ):
        """Test weight recommendation when no previous sets exist"""
        # Start a workout session
        started = await authenticated_client.post(
            "/api/v1/workouts/start",
            json={"name": "Test Workout", "type": "strength"},
        )
        assert started.status_code == 200, started.text
        session_id = started.json()["id"]

        # Try to get recommendation without any completed sets
        r = await authenticated_client.get(
            f"/api/v1/workouts/sessions/{session_id}/exercises/1/weight-recommendation"
        )
        assert r.status_code == 200, r.text
        data = r.json()
        assert data["recommendation"] == "no_data"
        assert "Нет данных" in data["message"]

    async def test_weight_recommendation_with_data_returns_recommendation(
        self,
        authenticated_client: AsyncClient,
    ):
        """Test weight recommendation when previous sets exist"""
        # Start a workout session
        started = await authenticated_client.post(
            "/api/v1/workouts/start",
            json={"name": "Test Workout", "type": "strength"},
        ...
        completed = await authenticated_client.post(
            f"/api/v1/workouts/complete?workout_id={session_id}",
            json={
                "duration": 30,
                "exercises": [
                    {
                        ...
                        "name": "Push-ups",
                        "sets_completed": [
                            {"set_number": 1, "completed": True, "reps": 10, "weight": 10, "rpe": 6},
                        ],
                    }
                ],
                "comments": "Test set",
        ...
        assert "увеличить вес" in data["message"].lower()
        assert data["suggested_weight"] == 10.25
