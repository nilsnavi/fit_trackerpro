...
class WorkoutSet(Base):
    ...
    set_type: Mapped[str] = mapped_column(
        String(16), nullable=False, default="working", server_default="working")
    ...
    planned_rest_seconds: Mapped[Optional[int]
                                 ] = mapped_column(Integer, nullable=True)
    actual_rest_seconds: Mapped[Optional[int]
                                ] = mapped_column(Integer, nullable=True)
    ...
    speed_kmh: Mapped[Optional[float]] = mapped_column(
        Numeric(8, 2), nullable=True)
    incline_pct: Mapped[Optional[float]] = mapped_column(
        Numeric(8, 2), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint("rir IS NULL OR (rir >= 0 AND rir <= 10)",
                        name="ck_workout_sets_rir_range"),
        CheckConstraint(
        ...
        CheckConstraint("duration IS NULL OR duration >= 0",
                        name="ck_workout_sets_duration_non_negative"),
        CheckConstraint(
        ...
        Index("ix_workout_sets_session_exercise_number",
