...
|---|---|---|---|---|
| `APP_NAME` | backend | `backend/.env.example`, `backend/app/core/config.py` | `FitTracker Pro` | опционально |
| `ENVIRONMENT` | backend | `backend/.env.example`, `backend/.env.production.example`, `backend/app/core/config.py`, `docker-compose*.yml` | `development` / `production` | обязательно для prod |
| `DEBUG` | backend | `backend/.env.example`, `backend/.env.production.example`, `backend/app/core/config.py`, `docker-compose*.yml` | `true` / `false` | обязательно для prod (`false`) |
| `DATABASE_URL` | backend | `backend/.env*.example`, `backend/app/core/config.py`, `docker-compose*.yml`, `.github/workflows/test.yml`, `.github/workflows/migrate.yml` | `postgresql+asyncpg://...` | обязательно |
| `DATABASE_URL_SYNC` | backend | `backend/.env*.example`, `backend/app/core/config.py`, `docker-compose*.yml`, `.github/workflows/test.yml`, `.github/workflows/migrate.yml` | `postgresql://...` | опционально (но фактически используется) |
| `REDIS_URL` | backend | `backend/.env*.example`, `backend/app/core/config.py`, `docker-compose*.yml`, `.github/workflows/test.yml` | `redis://localhost:6379/0` / `redis://redis:6379/0` | опционально (есть fallback) |
| `ANALYTICS_CACHE_ENABLED` | backend | `backend/.env*.example`, `backend/app/core/config.py` | `true` | опционально |
| `ANALYTICS_CACHE_TTL_SECONDS` | backend | `backend/.env*.example`, `backend/app/core/config.py` | `120` | опционально |
| `ANALYTICS_MEMORY_CACHE_ENABLED` | backend | `backend/.env*.example`, `backend/app/core/config.py` | `true` | опционально |
| `ANALYTICS_MEMORY_CACHE_TTL_SECONDS` | backend | `backend/.env*.example`, `backend/app/core/config.py` | `20` | опционально |
| `ANALYTICS_DEFAULT_MAX_EXERCISES` | backend | `backend/.env*.example`, `backend/app/core/config.py` | `30` | опционально |
| `ANALYTICS_MAX_EXERCISES_HARD_LIMIT` | backend | `backend/.env*.example`, `backend/app/core/config.py` | `100` | опционально |
| `ANALYTICS_DEFAULT_MAX_DATA_POINTS` | backend | `backend/.env*.example`, `backend/app/core/config.py` | `120` | опционально |
| `ANALYTICS_MAX_DATA_POINTS_HARD_LIMIT` | backend | `backend/.env*.example`, `backend/app/core/config.py` | `365` | опционально |
| `TELEGRAM_BOT_TOKEN` | backend | `backend/.env*.example`, `backend/app/core/config.py`, `docker-compose*.yml`, `.github/workflows/test.yml`, `.github/workflows/deploy.yml` | `your_bot_token_here` | обязательно для Telegram-функций |
| `TELEGRAM_WEBAPP_URL` | backend/frontend bridge | `backend/.env*.example`, `backend/app/core/config.py`, `docker-compose*.yml`, `.github/workflows/test.yml`, `.github/workflows/deploy.yml` | `https://your-domain.com` | обязательно в prod |
| `SECRET_KEY` | backend | `backend/.env*.example`, `backend/app/core/config.py`, `docker-compose*.yml`, `.github/workflows/test.yml`, `.github/workflows/deploy.yml` | `your_secret_key_here...` | обязательно |
| `ALGORITHM` | backend | `backend/.env.example`, `backend/app/core/config.py` | `HS256` | опционально |
| `ACCESS_TOKEN_EXPIRE_MINUTES` | backend | `backend/.env.example`, `backend/app/core/config.py` | `30` | опционально |
| `ALLOWED_ORIGINS` | backend | `backend/.env*.example`, `backend/app/core/config.py`, `docker-compose*.yml`, `.github/workflows/deploy.yml` | `http://localhost:3000,...` | обязательно в prod (не `*`) |
...
Эти переменные присутствуют в `backend/.env.example`, но в текущем `backend/app/core/config.py` не объявлены:
...
- сравнивает ключи из `backend/.env.example` и поля `Settings` (`backend/app/core/config.py`);
